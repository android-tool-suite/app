...
import com.androidtoolsuite.app.plugin.runtime.BackgroundTaskCall;
...
public final class PluginTaskWorker extends Worker {
    ...
    public Result doWork() {
        String pluginId = clean(getInputData().getString(SchedulerService.KEY_PLUGIN_ID));
        String taskId = clean(getInputData().getString(SchedulerService.KEY_TASK_ID));
        String generation = clean(getInputData().getString(SchedulerService.KEY_GENERATION));
        String runId = clean(getInputData().getString(SchedulerService.KEY_RUN_ID));
        String source = clean(getInputData().getString(SchedulerService.KEY_SOURCE));
        boolean scheduled = getInputData().getBoolean(SchedulerService.KEY_SCHEDULED, false);
        if (pluginId.isEmpty() || taskId.isEmpty() || generation.isEmpty()) return Result.failure();

        PluginRuntime runtime;
        try {
            runtime = PluginRuntime.get(getApplicationContext());
        } catch (RuntimeException error) {
            return Result.retry();
        }
        TaskRunStore store = runtime.taskRuns();
        ...
        RuntimePluginManifest.Task task = SchedulerService.task(installed.manifest, taskId);
        RuntimePluginManifest.BackgroundEntry entry = task == null
                ? null : SchedulerService.backgroundEntry(installed.manifest, task);
        ...
        try {
            ...
            TaskRunStore.Run run = store.read(pluginId, taskId, runId);
            ...
            TaskRunStore.Lease lease = store.acquire(
                    pluginId,
                    taskId,
                    runId,
                    maxConcurrency,
            ...
            try (TaskRunStore.Lease ignored = lease) {
                int attempt = getRunAttemptCount() + 1;
    ...
    public void onStopped() {
        super.onStopped();
        TaskRunStore store = activeStore;
