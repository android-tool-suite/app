...
/** Runtime v2 Tool whose UI is a bounded component document rendered by the Host. */
class V2DeclarativeToolPlugin(
    ...
    @Volatile private var activeWebSession: V2WebSession? = null
    ...
    override fun createView(activity: android.app.Activity, host: PluginHost): View {
        val view = composePluginView(activity) {
            V2DeclarativeToolScreen(installed, actions, hostRevision.intValue) { session ->
    ...
    override fun createHomeWidgets(activity: android.app.Activity, host: PluginHost): List<HomeWidget> =
        V2RuntimeHomeWidgets.create(installed, actions, hostRevision)
...
@Composable
private fun V2DeclarativeToolScreen(
    installed: V2PackageStore.InstalledPlugin,
    actions: V2HostActions,
    hostRevision: Int,
    onWebSessionChanged: (V2WebSession?) -> Unit,
) {
...
private fun loadDocument(installed: V2PackageStore.InstalledPlugin): DeclarativeUiDocument {
