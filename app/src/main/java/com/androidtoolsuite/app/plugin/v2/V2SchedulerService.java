...
public final class V2SchedulerService implements CapabilityProvider {
    ...
    private final V2TaskRunStore runs;
    private final V2CapabilityRouter events;
    ...
    public V2SchedulerService(
            Context context,
            V2PackageStore packages,
            V2TaskRunStore runs,
            V2CapabilityRouter events
    ) {
    ...
    public JSONObject call(CapabilityCall call) throws CapabilityFailure {
        V2PackageStore.InstalledPlugin installed = requireEnabled(call.pluginId);
    ...
    public synchronized void syncEnabledPlugins() {
        ...
        for (V2PackageStore.InstalledPlugin installed : packages.load()) {
    ...
    public void syncPlugin(String pluginId) {
        V2PackageStore.InstalledPlugin installed = packages.find(pluginId);
    ...
    JSONObject enqueueNow(
        ...
        try {
            V2TaskRunStore.Run run = runs.create(
                    pluginId,
    ...
    private OneTimeWorkRequest oneTimeRequest(
        ...
        OneTimeWorkRequest.Builder builder = new OneTimeWorkRequest.Builder(V2RuntimeWorker.class)
    ...
    private V2PackageStore.InstalledPlugin requireEnabled(String pluginId) throws CapabilityFailure {
        V2PackageStore.InstalledPlugin installed = packages.find(pluginId);
        if (installed == null || !installed.enabled) {
