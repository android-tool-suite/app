...
public class CapabilityRouterTest {
    @Test
    public void supportsExactComparatorCaretAndTildeRanges() {
        assertTrue(CapabilityRouter.versionSatisfied("1.4.2", "1.4.2"));
        assertTrue(CapabilityRouter.versionSatisfied("1.4.2", ">=1.0.0 <2.0.0"));
        assertTrue(CapabilityRouter.versionSatisfied("1.9.0", "^1.2.0"));
        assertFalse(CapabilityRouter.versionSatisfied("2.0.0", "^1.2.0"));
        assertTrue(CapabilityRouter.versionSatisfied("1.4.9", "~1.4.0"));
        assertFalse(CapabilityRouter.versionSatisfied("1.5.0", "~1.4.0"));
    }
    ...
    public void routesOnlyDeclaredCapabilityAndDisposesRegistration() throws Exception {
        ...
        AutoCloseable registration = router.register(new CapabilityProvider() {
            ...
            @Override public JSONObject call(CapabilityCall call) throws com.androidtoolsuite.app.plugin.runtime.CapabilityFailure {
                try {
                    return new JSONObject().put("pluginId", call.pluginId);
                } catch (org.json.JSONException error) {
                    throw new com.androidtoolsuite.app.plugin.runtime.CapabilityFailure("INTERNAL", error.getMessage(), false);
                }
            }
        }, "test", 1);
