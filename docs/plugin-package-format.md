...
主体 2.0.0 完成旧运行时管理代码退出：历史安装记录不再参与插件列表、启停、依赖、导出或同 ID 安装冲突检查。设备上的旧文件和业务数据不会自动删除；未迁移的数据需先使用支持 Migration Bridge 的旧版本导出，再通过数据备份恢复。旧宿主迁移包只恢复兼容的应用设置，不恢复旧插件包及其启用状态。
